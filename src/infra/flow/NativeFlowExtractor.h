...
/** Native C++ flow feature extractor producing LSNM2024-compatible 77-feature vectors. */
...
public:
     ...
     * @param timeoutUs  Timeout in microseconds; flows idle longer than this are finalized.
     */
...
private:
    ...
    struct ParsedPacket {
        ...
        const std::uint8_t* transportHeader = nullptr;  ///< Pointer to TCP/UDP/ICMP header
        std::uint32_t transportHeaderLen = 0;
        std::uint32_t ipIhl = 0;
    };

    /// Parse Ethernet + IP + transport headers. Returns false if packet should be skipped.
    [[nodiscard]] static bool parsePacketHeaders(const std::uint8_t* data, std::uint32_t len,
                                            ParsedPacket& pkt);

    /// Parse Ethernet frame and IP header, producing transport layer start pointer.
    [[nodiscard]] static bool parseEthernetAndIp(const std::uint8_t* data, std::uint32_t len,
                                            ParsedPacket& pkt,
    ...
    [[nodiscard]] static bool parseTransportHeader(ParsedPacket& pkt,
                                              const std::uint8_t* transportStart,
    ...
    /// Update forward-direction TCP state (PSH/URG flags, init window, segment tracking).
    ...
    static void updateActiveIdle(FlowStats& stats, std::int64_t timestampUs,
                                  std::int64_t prevLastTimeUs, std::int64_t flowGapUs);
