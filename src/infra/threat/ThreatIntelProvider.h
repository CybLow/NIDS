...
///
/// Feed files are plain text, one entry per line, '#' comments, blank lines
/// ignored. The file's basename (without extension) becomes the feed name.
///
/// Performance:
...
public:
  ...
  loadFeeds(const std::filesystem::path &feedDirectory) override;
...
private:
  ...
  /// Parse a single feed entry (IP or CIDR) and store it. Returns 1 on success,
  /// 0 on failure.
  std::size_t parseAndStoreEntry(const std::string &entry,
                                 const std::string &feedName);
