...
void PcapCaptureWorker::doCapture() {
    ...
    if (!bpfFilter_.empty()) {
        ...
        if (pcap_compile(handle_.get(), &fp, bpfFilter_.c_str(), 0, PCAP_NETMASK_UNKNOWN) != -1) {
            if (pcap_setfilter(handle_.get(), &fp) == -1) {
                emit captureError(
                    QString("Could not install filter: %1").arg(pcap_geterr(handle_.get())));
            }
            pcap_freecode(&fp);
        } else {
            emit captureError(
                QString("Could not compile filter: %1").arg(pcap_geterr(handle_.get())));
        }
    }
    ...
    if (!dumpFile_.empty()) {
        ...
        if (rawDumper) {
            ...
            emit captureError(
                QString("Error opening dump file: %1").arg(pcap_geterr(handle_.get())));
        }
    }

    capturing_.store(true);
    pcap_loop(handle_.get(), 0, packetCallback, reinterpret_cast<unsigned char*>(this));
...
void PcapCaptureWorker::processPacket(const struct pcap_pkthdr* pkthdr,
    ...
    if (dumper_) {
        pcap_dump(reinterpret_cast<unsigned char*>(dumper_.get()), pkthdr, packet);
    }
...
bool PcapCapture::initialize(const std::string& interface, const std::string& bpfFilter) {
    interface_ = interface;
    bpfFilter_ = bpfFilter;
    return true;
}
...
bool PcapCapture::isCapturing() const {
