...
bool SyslogSink::start() {
    ...
    const int type = (config_.transport == SyslogTransport::Tcp)
                         ? SOCK_STREAM
                         : SOCK_DGRAM;
    ...
    spdlog::info("SyslogSink started: {}:{} ({})",
...
std::string SyslogSink::formatMessage(
...
std::string SyslogSink::formatRfc5424(
    std::size_t flowIndex,
    const core::DetectionResult& result,
    const core::FlowInfo& flow) const {
    ...
    return fmt::format(
        ...
        "protocol=\"{}\" verdict=\"{}\" confidence=\"{:.4f}\" "
        "combinedScore=\"{:.4f}\" detectionSource=\"{}\" "
        "tiMatches=\"{}\" ruleMatches=\"{}\"] "
        "flow #{}: {} (confidence {:.1f}%)",
        ...
        result.mlResult.confidence, result.combinedScore, sourceStr,
        tiFeeds, rules,
        flowIndex, verdictStr, result.mlResult.confidence * 100.0f);
}
...
int SyslogSink::syslogSeverity(float combinedScore) noexcept {
    ...
    if (combinedScore >= 0.85f) return 2;  // Critical
    if (combinedScore >= 0.7f)  return 3;  // Error
    if (combinedScore >= 0.5f)  return 4;  // Warning
    if (combinedScore >= 0.3f)  return 5;  // Notice
