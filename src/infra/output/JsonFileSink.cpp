    ...
    if (config_.outputPath.has_parent_path()) {
        ...
        if (ec) {
            spdlog::error("JsonFileSink: cannot create directory '{}': {}",
                          config_.outputPath.parent_path().string(),
                          ec.message());
            return false;
        }
    }
...
std::string JsonFileSink::toJson(
    ...
    nlohmann::json j = {
        ...
        {"detection", {
            {"finalVerdict", std::string{core::attackTypeToString(result.finalVerdict)}},
            {"combinedScore", result.combinedScore},
            {"detectionSource", std::string{core::detectionSourceToString(result.detectionSource)}},
            {"isFlagged", result.isFlagged()},
            {"ml", {
                {"classification", std::string{core::attackTypeToString(result.mlResult.classification)}},
...
void JsonFileSink::rotateIfNeeded() {
    ...
    if (currentSize_ < config_.maxFileSizeBytes) return;
