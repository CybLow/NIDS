...
/// Unified detection result combining ML, threat intelligence, and heuristic
/// signals.
///
/// Produced by HybridDetectionService after evaluating a flow through all three
/// detection layers. Provides full traceability: the final verdict, which
...
struct DetectionResult {
  ...
  [[nodiscard]] float maxRuleSeverity() const noexcept {
    ...
    auto it = std::ranges::max_element(
        ruleMatches, {}, [](const auto &r) { return r.severity; });
    return it->severity;
  }
