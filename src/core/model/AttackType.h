...
inline constexpr std::array<std::string_view, kAttackTypeTotal>
    kAttackTypeNames = {{
        "Benign",                // 0
        "MITM ARP Spoofing",     // 1
        "SSH Brute Force",       // 2
...
attackTypeToString(AttackType type) noexcept {
  if (auto idx = static_cast<std::size_t>(type); // NOLINT NOSONAR
      idx < kAttackTypeNames.size())
