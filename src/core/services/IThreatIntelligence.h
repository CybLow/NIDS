...
public:
    ...
    [[nodiscard]] virtual std::size_t loadFeeds(const std::string& feedDirectory) = 0;

    /// Check if an IPv4 address (dotted-decimal string) appears in any loaded feed.
    [[nodiscard]] virtual ThreatIntelLookup lookup(std::string_view ip) const = 0;

    /// Check if an IPv4 address (32-bit host-byte-order integer) appears in any feed.
