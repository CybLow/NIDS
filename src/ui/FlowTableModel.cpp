...
namespace {
    ...
    QVariant fmtSrcIp(int /*row*/, const FlowRow& r)  { return QString::fromStdString(r.metadata.srcIp); }
    QVariant fmtSrcPort(int /*row*/, const FlowRow& r) { return r.metadata.srcPort; }
    QVariant fmtDstIp(int /*row*/, const FlowRow& r)   { return QString::fromStdString(r.metadata.dstIp); }
    QVariant fmtDstPort(int /*row*/, const FlowRow& r) { return r.metadata.dstPort; }
    ...
    QVariant fmtConfidence(int /*row*/, const FlowRow& r) {
        return QString::number(
            static_cast<double>(r.result.mlResult.confidence) * 100.0, 'f', 1) + "%";
    }
...
QVariant FlowTableModel::alignmentData(int column) {
    ...
    return static_cast<int>(rightAligned
...
QColor FlowTableModel::severityColor(float combinedScore, bool isFlagged) noexcept {
...
QString FlowTableModel::protocolToString(std::uint8_t protocol) noexcept {
    switch (protocol) {
        ...
        case 17: return "UDP";
        case 1:  return "ICMP";
        default: return QString("Other (%1)").arg(protocol);
    }
}
