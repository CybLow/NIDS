...
void DetectionDetailWidget::setupUi() {
    ...
    flowGroup_ = new QGroupBox("Flow Information", this);
    auto* flowLayout = new QVBoxLayout(flowGroup_);
    flowSrcLabel_ = new QLabel(flowGroup_);
    flowDstLabel_ = new QLabel(flowGroup_);
    flowProtocolLabel_ = new QLabel(flowGroup_);
    flowDurationLabel_ = new QLabel(flowGroup_);
    flowPacketsLabel_ = new QLabel(flowGroup_);
    ...
    verdictGroup_ = new QGroupBox("Detection Verdict", this);
    auto* verdictLayout = new QVBoxLayout(verdictGroup_);
    verdictLabel_ = new QLabel(verdictGroup_);
    ...
    combinedScoreLabel_ = new QLabel(verdictGroup_);
    detectionSourceLabel_ = new QLabel(verdictGroup_);
    ...
    // -- ML section --
    mlGroup_ = new QGroupBox("ML Classifier", this);
    auto* mlLayout = new QVBoxLayout(mlGroup_);
    mlClassLabel_ = new QLabel(mlGroup_);
    mlConfidenceLabel_ = new QLabel(mlGroup_);
    mlLayout->addWidget(mlClassLabel_);
    mlLayout->addWidget(mlConfidenceLabel_);

    probabilityTable_ = new QTableWidget(kProbabilityTableRows, kProbabilityTableCols, mlGroup_);
    probabilityTable_->setHorizontalHeaderLabels({"Attack Type", "Probability"});
    probabilityTable_->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
    ...
    tiGroup_ = new QGroupBox("Threat Intelligence Matches", this);
    auto* tiLayout = new QVBoxLayout(tiGroup_);
    tiTable_ = new QTableWidget(0, kTiTableCols, tiGroup_);
    ...
    rulesGroup_ = new QGroupBox("Heuristic Rule Matches", this);
    auto* rulesLayout = new QVBoxLayout(rulesGroup_);
    rulesTable_ = new QTableWidget(0, kRulesTableCols, rulesGroup_);
...
void DetectionDetailWidget::setResult(const nids::core::DetectionResult& result,
    ...
    if (metadata) {
        ...
        flowPacketsLabel_->setText(
            QString("Packets: %1 fwd / %2 bwd")
    ...
    verdictLabel_->setText(QString("Verdict: %1").arg(
        QString::fromUtf8(verdictStr.data(), static_cast<int>(verdictStr.size()))));
    combinedScoreLabel_->setText(
        QString("Combined Score: %1")
            .arg(static_cast<double>(result.combinedScore), 0, 'f', kScorePrecision));
    auto sourceStr = nids::core::detectionSourceToString(result.detectionSource);
    detectionSourceLabel_->setText(QString("Detection Source: %1").arg(
        QString::fromUtf8(sourceStr.data(), static_cast<int>(sourceStr.size()))));

    // -- ML classifier --
    auto mlStr = nids::core::attackTypeToString(result.mlResult.classification);
    mlClassLabel_->setText(QString("Classification: %1").arg(
        QString::fromUtf8(mlStr.data(), static_cast<int>(mlStr.size()))));
    mlConfidenceLabel_->setText(
        QString("Confidence: %1%")
            .arg(static_cast<double>(result.mlResult.confidence) * kPercentMultiplier,
                 0, 'f', 1));

    // Populate probability distribution table
    for (int i = 0; i < nids::core::kAttackTypeCount; ++i) {
        auto typeName = nids::core::attackTypeToString(
            nids::core::attackTypeFromIndex(i));
        auto* nameItem = new QTableWidgetItem(
            QString::fromUtf8(typeName.data(), static_cast<int>(typeName.size())));
        probabilityTable_->setItem(i, 0, nameItem);

        auto prob = static_cast<double>(result.mlResult.probabilities[
    ...
    for (int i = 0; i < tiCount; ++i) {
        ...
        tiTable_->setItem(i, 0, new QTableWidgetItem(
            QString::fromStdString(match.ip)));
        tiTable_->setItem(i, 1, new QTableWidgetItem(
            QString::fromStdString(match.feedName)));
        tiTable_->setItem(i, 2, new QTableWidgetItem(
            match.isSource ? "Source" : "Destination"));
    }
    ...
    for (int i = 0; i < rulesCount; ++i) {
        ...
        rulesTable_->setItem(i, 0, new QTableWidgetItem(
            QString::fromStdString(rule.ruleName)));
        rulesTable_->setItem(i, 1, new QTableWidgetItem(
            QString::number(static_cast<double>(rule.severity), 'f', kProbPrecision)));
        rulesTable_->setItem(i, 2, new QTableWidgetItem(
            QString::fromStdString(rule.description)));
    }
...
void DetectionDetailWidget::clearResult() {
    ...
    for (int i = 0; i < nids::core::kAttackTypeCount; ++i) {
        auto typeName = nids::core::attackTypeToString(
            nids::core::attackTypeFromIndex(i));
        probabilityTable_->setItem(i, 0, new QTableWidgetItem(
            QString::fromUtf8(typeName.data(), static_cast<int>(typeName.size()))));
        auto* probItem = new QTableWidgetItem("—");
        probItem->setTextAlignment(Qt::AlignRight | Qt::AlignVCenter);
        probabilityTable_->setItem(i, 1, probItem);
