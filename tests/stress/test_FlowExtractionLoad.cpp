...
protected:
    ...
    void SetUp() override {
        ...
        std::string filename = std::string("nids_stress_fe_") + info->name() + ".pcap";
...
TEST_F(FlowExtractionLoadTest, flowSplitting_megaFlow) {
    ...
    spdlog::info("Mega-flow (1 flow, 1000 pkts): {} sub-flows extracted (expected >= {})",
...
TEST_F(FlowExtractionLoadTest, flowTimeout_oldFlowsEvicted) {
    ...
    NativeFlowExtractor extractor;
    extractor.setFlowTimeout(50'000'000);  // 50 seconds -> each packet creates new flow

    auto features = extractor.extractFeatures(pcapPath_);

    spdlog::info("Flow timeout test: {} packets -> {} flows (timeout=50s, IAT=100s)",
...
TEST_F(FlowExtractionLoadTest, largeScale_10kFlows_100kPackets) {
    ...
    spdlog::info("10k flows / 100k packets: {:.1f} ms, {} flows, {:.0f} flows/sec",
                 elapsedMs, features.size(),
