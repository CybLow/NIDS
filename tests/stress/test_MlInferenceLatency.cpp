...
std::vector<std::vector<float>> generateFeatures(std::size_t count,
                                                 unsigned seed = 42) {
...
TEST_F(MlInferenceLatencyTest, predict_10kFlows_throughput) {
  ...
  spdlog::info(
      "predict() 10k flows: {:.1f} ms total, {:.1f} us/flow, {:.0f} flows/sec, "
      "{} attacks detected",
...
TEST_F(MlInferenceLatencyTest, predictWithConfidence_10kFlows_throughput) {
  ...
  double flowsPerSec = static_cast<double>(kFlows) / (elapsedMs / 1000.0);
  float avgConf =
      std::accumulate(confidences.begin(), confidences.end(), 0.0f) /
      static_cast<float>(confidences.size());
  spdlog::info(
      "predictWithConfidence() 10k flows: {:.1f} ms, {:.0f} flows/sec, "
      "avg confidence={:.3f}",
...
TEST_F(MlInferenceLatencyTest, predict_latencyDistribution) {
  ...
  spdlog::info("Latency distribution (us): p50={:.1f}, p95={:.1f}, p99={:.1f}, "
               "max={:.1f}",
               p50, p95, p99, maxLat);
...
TEST_F(OnnxInferenceStressTest, predict_1kFlows_realModel) {
  ...
  spdlog::info("ONNX predict() 1k flows: {:.1f} ms total, {:.2f} ms/flow, "
               "{:.0f} flows/sec",
