...
class ScopedTimer {
public:
  explicit ScopedTimer(double &outMs) : out_(outMs) {}
  ~ScopedTimer() {
...
/** Read current VmRSS (resident set size) in kilobytes from /proc. Returns 0 on
 * failure. */
 ...
 *
 * Each packet is a minimal Ethernet(14) + IPv4(20) + TCP(20) + optional
 * payload. Source IPs rotate through 10.0.{flowId/256}.{flowId%256}, dest is
 * always 10.1.0.1. Source port is 40000 + flowId, dest port is 80. Timestamps
 * increment by `iatUs` microseconds between packets.
 *
 * @param outPath      Path to write the pcap file.
 * @param packetCount  Total number of packets to generate.
 * @param flowCount    Number of distinct flows (packets round-robin across
 * flows).
 ...
 */
inline void generatePcap(const std::string &outPath, std::uint32_t packetCount,
                         std::uint32_t flowCount, std::uint32_t payloadSize = 0,
                         std::int64_t iatUs = 1000) {
  std::ofstream ofs(outPath, std::ios::binary);

  // Pcap global header (24 bytes): magic, v2.4, snaplen=65535,
  // linktype=ETHERNET
...
/** Lightweight mock analyzer that returns deterministic results without ONNX
 * Runtime. */
...
public:
  ...
  lookup(std::string_view ip) const override {
    bool found = std::ranges::any_of(
        blacklist_, [ip](const auto &entry) { return entry == ip; });
    return found ? core::ThreatIntelLookup{true, "test_feed"}
                 : core::ThreatIntelLookup{false, ""};
  }
  [[nodiscard]] core::ThreatIntelLookup
  lookup(std::uint32_t /*ip*/) const override {
    return {false, ""};
  }
