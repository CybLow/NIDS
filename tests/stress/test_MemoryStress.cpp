...
protected:
    ...
    void SetUp() override {
        ...
        std::string filename = std::string("nids_stress_mem_") + info->name() + ".pcap";
...
TEST_F(MemoryStressTest, repeatedExtraction_noMemoryLeak) {
    ...
    if (rssBaseline > 0) {
        ...
        auto growth = static_cast<std::int64_t>(rssEnd) - static_cast<std::int64_t>(rssBaseline);
        double growthMb = static_cast<double>(growth) / 1024.0;

        spdlog::info("Memory: baseline={} KB, final={} KB, growth={:.1f} MB across {} iterations",
...
TEST_F(MemoryStressTest, largeFlowTable_memoryBounded) {
    ...
    spdlog::info("Large flow table (10k flows): {} features extracted", features.size());

    if (rssBefore > 0 && rssAfter > 0) {
        auto growth = static_cast<std::int64_t>(rssAfter) - static_cast<std::int64_t>(rssBefore);
        double growthMb = static_cast<double>(growth) / 1024.0;
...
TEST_F(MemoryStressTest, sustainedEvaluation_memoryStable) {
    ...
    for (int i = 0; i < kIterations; ++i) {
        ...
        pred.classification = (i % 3 == 0)
            ? nids::core::AttackType::DdosIcmp
    ...
    if (rssBaseline > 0 && !checkpoints.empty()) {
        spdlog::info("Sustained evaluation memory checkpoints (baseline={} KB):", rssBaseline);
        for (const auto& [iter, rss] : checkpoints) {
            auto growthKb = static_cast<std::int64_t>(rss) - static_cast<std::int64_t>(rssBaseline);
            spdlog::info("  iter={}: RSS={} KB (delta={:+d} KB)", iter, rss, growthKb);
        }
        ...
        EXPECT_LT(growthMb, 10.0)
            << "Memory grew " << growthMb << " MB during sustained evaluation — possible leak";
    }
}
...
TEST_F(MemoryStressTest, predictorMemory_noAccumulation) {
    ...
    std::vector<float> features(static_cast<std::size_t>(kFlowFeatureCount), 0.5f);
    ...
    if (rssBefore > 0 && rssAfter > 0) {
        double growthMb = static_cast<double>(
            static_cast<std::int64_t>(rssAfter) - static_cast<std::int64_t>(rssBefore)) / 1024.0;
