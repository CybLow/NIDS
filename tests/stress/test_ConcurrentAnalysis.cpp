...
TEST_F(ConcurrentAnalysisTest, singleThread_10kEvaluations) {
    ...
    spdlog::info("Single-thread 10k evaluations: {:.1f} ms, {:.0f} eval/sec, {} attacks",
...
TEST_F(ConcurrentAnalysisTest, multiThread_4threads_10kEach) {
    ...
    spdlog::info("{} threads x {} flows: {:.1f} ms, {:.0f} eval/sec, {} attacks, {} errors",
                 kThreads, kFlowsPerThread, elapsedMs, epsec,
                 totalAttacks.load(), totalErrors.load());
...
TEST_F(ConcurrentAnalysisTest, multiThread_8threads_highContention) {
    ...
    std::size_t totalFlows = kFlowsPerThread * kThreads;
    double epsec = static_cast<double>(totalFlows) / (elapsedMs / 1000.0);
    spdlog::info("{} threads x {} flows (TI contention): {:.1f} ms, {:.0f} eval/sec, "
                 "{} TI matches, {} errors",
                 kThreads, kFlowsPerThread, elapsedMs, epsec,
                 tiMatches.load(), totalErrors.load());
...
TEST_F(ConcurrentAnalysisTest, weightChangeUnderLoad) {
    ...
    EXPECT_EQ(totalErrors.load(), 0u) << "Errors during concurrent weight changes";
