...
protected:
    ...
    void SetUp() override {
        ...
        std::string filename = std::string("nids_stress_tp_") + info->name() + ".pcap";
...
TEST_F(PacketThroughputTest, parse10kPackets_singleFlow) {
    ...
    spdlog::info("10k packets / 1 flow: {:.1f} ms, {:.0f} pps, {} flow(s) extracted",
...
TEST_F(PacketThroughputTest, parse100kPackets_withPayload) {
    ...
    double pps = static_cast<double>(kPackets) / (elapsedMs / 1000.0);
    double mbps = static_cast<double>(kPackets) * (54.0 + kPayload) / (elapsedMs / 1000.0) / 1'000'000.0;
    spdlog::info("100k packets / 500 flows / 256B payload: {:.1f} ms, {:.0f} pps, {:.1f} MB/s",
                 elapsedMs, pps, mbps);
