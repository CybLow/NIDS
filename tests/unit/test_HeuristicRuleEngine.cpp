...
TEST_F(HeuristicRuleEngineTest, evaluate_icmpFlood_fires) {
    ...
    flow.flowDurationUs = 1'000'000; // 1 s → rate = 1000/1.0 = 1000 pkt/s, above 100 threshold
    auto results = engine_.evaluate(flow);
...
TEST_F(HeuristicRuleEngineTest, evaluate_highPacketRate_fires) {
    ...
    flow.flowDurationUs = 500'000; // 0.5 s → rate = 10000/0.5 = 20000 pkt/s, above 10000 threshold
    auto results = engine_.evaluate(flow);
...
TEST_F(HeuristicRuleEngineTest, evaluate_suspiciousSrcPort_fires) {
    ...
    EXPECT_TRUE(found) << "Expected suspicious_port rule to fire for src port 31337";
}
...
TEST_F(HeuristicRuleEngineTest, evaluate_bothPortsSuspicious_higherSeverity) {
    ...
    EXPECT_TRUE(found) << "Expected suspicious_port rule for both suspicious ports";
}
...
TEST_F(HeuristicRuleEngineTest, evaluate_highPacketRate_zeroDuration_doesNotFire) {
    auto flow = makeBenignFlow();
    flow.totalFwdPackets = 5000;
    flow.totalBwdPackets = 5000;
...
TEST_F(HeuristicRuleEngineTest, evaluate_highPacketRate_belowMinPkts_doesNotFire) {
    auto flow = makeBenignFlow();
...
TEST_F(HeuristicRuleEngineTest, evaluate_multipleRulesFire) {
    ...
    for (const auto& r : results) {
        if (r.ruleName == "suspicious_port") hasSuspPort = true;
        if (r.ruleName == "syn_flood") hasSynFlood = true;
        if (r.ruleName == "high_packet_rate") hasHighRate = true;
    }
...
TEST_F(HeuristicRuleEngineTest, evaluate_bruteForce_belowRate_doesNotFire) {
    ...
    flow.flowDurationUs = 10'000'000; // 10s → rate = 25/10 = 2.5, below 10 threshold
    auto results = engine_.evaluate(flow);
...
TEST_F(HeuristicRuleEngineTest, evaluate_highPacketRate_belowRate_doesNotFire) {
    ...
    flow.flowDurationUs = 5'000'000; // 5s → rate = 120/5 = 24, below 10000 threshold
    auto results = engine_.evaluate(flow);
...
TEST_F(HeuristicRuleEngineTest, evaluate_resetFlood_zeroTotalPackets_doesNotFire) {
    auto flow = makeBenignFlow();
    flow.protocol = "TCP";
...
TEST_F(HeuristicRuleEngineTest, evaluate_bothSuspiciousPorts_severityHigh) {
    ...
    EXPECT_TRUE(found) << "suspicious_port rule should fire when both ports are suspicious";
}
