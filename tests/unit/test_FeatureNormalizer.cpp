...
protected:
  ...
  void SetUp() override { fs::create_directories(testDir_); }
  ...
  std::string writeMetadata(const std::string &name,
  ...
  std::string writeValidMetadata(const std::string &name, std::size_t nFeatures,
    ...
    auto json = std::format(
        R"({{"normalization":{{"method":"standard_scaler","means":{},"stds":{},"clip_value":{}}}}})",
        means, stds, clipValue);
...
TEST_F(FeatureNormalizerTest,
       normalize_dimensionMismatch_returnsInputUnchanged) {
...
TEST_F(FeatureNormalizerTest, normalize_full77Features_succeeds) {
  ...
  for (std::size_t i = 0; i < kFlowFeatureCount; ++i) {
    float expected =
        (input[i] - static_cast<float>(i)) / (1.0f + static_cast<float>(i));
