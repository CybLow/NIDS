...
/// PCAP global header (libpcap format, little-endian, version 2.4, link type 1 = Ethernet)
...
std::vector<std::uint8_t> buildTcpPacket(
...
std::vector<std::uint8_t> buildUdpPacket(
    const char* srcIp, const char* dstIp,
    std::uint16_t srcPort, std::uint16_t dstPort,
    std::uint16_t payloadSize = 0)
{
...
std::vector<std::uint8_t> buildIcmpPacket(
    const char* srcIp, const char* dstIp,
...
std::vector<std::uint8_t> buildVlanTcpPacket(
    const char* srcIp, const char* dstIp,
    std::uint16_t srcPort, std::uint16_t dstPort,
...
std::string writePcapFile(const std::string& name,
    ...
    // Global header
    buf.insert(buf.end(), std::begin(kPcapGlobalHeader), std::end(kPcapGlobalHeader));
...
TEST(NativeFlowExtractor, ExtractFeatures_bidirectionalFlow) {
    ...
    auto path = writePcapFile("nfe_bidir.pcap", {
        {fwd, 0, 0},
...
TEST(NativeFlowExtractor, ExtractFeatures_tcpFinCompletesFlow) {
    ...
    auto synack = buildTcpPacket("10.0.0.2", "10.0.0.1", 80, 5000, 0x12);  // SYN+ACK
...
TEST(NativeFlowExtractor, ExtractFeatures_flowTimeoutEviction) {
    ...
    auto path = writePcapFile("nfe_timeout.pcap", {
        {pkt1, 0, 0},
...
TEST(NativeFlowExtractor, ExtractFeatures_tcpFlagsCounted) {
    ...
    auto synack = buildTcpPacket("10.0.0.2", "10.0.0.1", 80, 5000, 0x12);      // SYN+ACK
    auto ack    = buildTcpPacket("10.0.0.1", "10.0.0.2", 5000, 80, 0x10);      // ACK
    auto pshack = buildTcpPacket("10.0.0.1", "10.0.0.2", 5000, 80, 0x18);      // PSH+ACK
    auto finack = buildTcpPacket("10.0.0.1", "10.0.0.2", 5000, 80, 0x11);      // FIN+ACK
...
TEST(NativeFlowExtractor, ExtractFeatures_nonIpv4Skipped) {
    ...
    arpPkt[12] = 0x08; arpPkt[13] = 0x06;  // ARP
...
TEST(NativeFlowExtractor, ExtractFeatures_activeIdlePeriods) {
    ...
    auto path = writePcapFile("nfe_active_idle.pcap", {
        {pkt1, 0, 0},
...
TEST(NativeFlowExtractor, ExtractFeatures_truncatedTcpHeader_skipped) {
    // Build IPv4 packet where payload is too short for TCP header (IP=20 bytes, need +20 for TCP)
    // Ethernet(14) + IP(20) + partial TCP (10 bytes instead of 20) = 44
...
TEST(NativeFlowExtractor, ExtractFeatures_ipHeaderTooShort_skipped) {
    // Ethernet(14) + minimal data (IPv4 EtherType but only 10 bytes of IP payload)
...
TEST(NativeFlowExtractor, ExtractFeatures_vlanTooShort_skipped) {
    ...
    pkt[12] = 0x81; pkt[13] = 0x00;  // VLAN EtherType
...
TEST(NativeFlowExtractor, ExtractFeatures_urgAndCwrAndEceFlags) {
    ...
    auto bwd_urg_psh = buildTcpPacket("10.0.0.2", "10.0.0.1", 80, 5000, 0x28);  // PSH+URG

    auto path = writePcapFile("nfe_rare_flags.pcap", {
        {urg, 0, 0},
        {cwr, 0, 100'000},
        {ece, 0, 200'000},
...
TEST(NativeFlowExtractor, ExtractFeatures_backwardKeyTimeoutEviction) {
    ...
    auto pkt1 = buildTcpPacket("10.0.0.1", "10.0.0.2", 5000, 80, 0x02);
    // Packet B→A at t=700s (resolveFlow tries keyBwd={A,B} which matches, but timed out)
...
TEST(NativeFlowExtractor, FlowMetadata_singlePacket_zeroDuration) {
    ...
    EXPECT_FLOAT_EQ(meta[0].fwdPacketsPerSecond, 0.0f);
    EXPECT_FLOAT_EQ(meta[0].bwdPacketsPerSecond, 0.0f);
}
...
TEST(NativeFlowExtractor, ExtractFeatures_bwdBulkFlushedAtFlowCompletion) {
    ...
    auto bwdFin = buildTcpPacket("10.0.0.2", "10.0.0.1", 80, 5000, 0x11); // FIN+ACK

    auto path = writePcapFile("test_bwd_bulk_complete.pcap", {
        {fwd1, 0, 0},
        {bwd1, 0, 100'000},
        {bwd2, 0, 200'000},
        {bwdFin, 0, 300'000},
...
TEST(NativeFlowExtractor, ExtractFeatures_bwdBulkFlushedAtFinalize) {
    ...
    auto path = writePcapFile("test_bwd_bulk_finalize.pcap", {
        {fwd1, 0, 0},
        {bwd1, 0, 100'000},
        {bwd2, 0, 200'000},
        {bwd3, 0, 300'000},
...
TEST(NativeFlowExtractorTest, ExtractFeatures_tcpDataOffsetBelowMin_clampedTo20) {
    ...
    auto path = writePcapFile("test_tcp_low_offset.pcap",
        {{pkt, 1, 0}});
